...
impl DebugMode {
    pub(crate) fn new(raw: u32, fw: crate::headers::InternalFirmware) -> Self {
        match raw {
            0u32 => Self::None,
            1u32 if fw.is_betaflight() => Self::Cycletime,
            1u32 if fw.is_inav() => Self::Agl,
            2u32 if fw.is_betaflight() => Self::Battery,
            2u32 if fw.is_inav() => Self::FlowRaw,
            3u32 if fw.is_betaflight() => Self::GyroFiltered,
            3u32 if fw.is_inav() => Self::Flow,
            4u32 if fw.is_betaflight() => Self::Accelerometer,
            4u32 if fw.is_inav() => Self::Always,
            5u32 if fw.is_betaflight() => Self::Pidloop,
            5u32 if fw.is_inav() => Self::SagCompVoltage,
            6u32 if fw.is_betaflight() => Self::GyroScaled,
            6u32 if fw.is_inav() => Self::Vibe,
            7u32 if fw.is_betaflight() => Self::RcInterpolation,
            7u32 if fw.is_inav() => Self::Cruise,
            8u32 if fw.is_betaflight() => Self::Anglerate,
            8u32 if fw.is_inav() => Self::RemFlightTime,
            9u32 if fw.is_betaflight() => Self::EscSensor,
            9u32 if fw.is_inav() => Self::Smartaudio,
            10u32 if fw.is_betaflight() => Self::Scheduler,
            10u32 if fw.is_inav() => Self::Acc,
            11u32 if fw.is_betaflight() => Self::Stack,
            11u32 if fw.is_inav() => Self::NavYaw,
            12u32 if fw.is_betaflight() => Self::EscSensorRpm,
            12u32 if fw.is_inav() => Self::Pcf8574,
            13u32 if fw.is_betaflight() => Self::EscSensorTmp,
            13u32 if fw.is_inav() => Self::DynamicGyroLpf,
            14u32 if fw.is_betaflight() => Self::Altitude,
            14u32 if fw.is_inav() => Self::AutoLevel,
            15u32 if fw.is_betaflight() => Self::Fft,
            15u32 if fw.is_inav() => Self::Altitude,
            16u32 if fw.is_betaflight() => Self::FftTime,
            16u32 if fw.is_inav() => Self::AutoTrim,
            17u32 if fw.is_betaflight() => Self::FftFreq,
            17u32 if fw.is_inav() => Self::AutoTune,
            18u32 if fw.is_betaflight() => Self::RxFrskySpi,
            18u32 if fw.is_inav() => Self::RateDynamics,
            19u32 if fw.is_betaflight() => Self::RxSfhssSpi,
            19u32 if fw.is_inav() => Self::Landing,
            20u32 if fw.is_betaflight() => Self::GyroRaw,
            21u32 if fw.is_betaflight() => Self::DualGyroRaw,
            22u32 if fw.is_betaflight() => Self::DualGyroDiff,
            23u32 if fw.is_betaflight() => Self::Max745Signal,
            24u32 if fw.is_betaflight() => Self::Max745Spiclock,
            25u32 if fw.is_betaflight() => Self::Sbus,
            26u32 if fw.is_betaflight() => Self::Fport,
            27u32 if fw.is_betaflight() => Self::Rangefinder,
            28u32 if fw.is_betaflight() => Self::RangefinderQuality,
            29u32 if fw.is_betaflight() => Self::LidarTf,
            30u32 if fw.is_betaflight() => Self::AdcInternal,
            31u32 if fw.is_betaflight() => Self::RunawayTakeoff,
            32u32 if fw.is_betaflight() => Self::Sdio,
            33u32 if fw.is_betaflight() => Self::CurrentSensor,
            34u32 if fw.is_betaflight() => Self::Usb,
            35u32 if fw.is_betaflight() => Self::Smartaudio,
            36u32 if fw.is_betaflight() => Self::Rth,
            37u32 if fw.is_betaflight() => Self::ItermRelax,
            38u32 if fw.is_betaflight() => Self::AcroTrainer,
            39u32 if fw.is_betaflight() => Self::RcSmoothing,
            40u32 if fw.is_betaflight() => Self::RxSignalLoss,
            41u32 if fw.is_betaflight() => Self::RcSmoothingRate,
            42u32 if fw.is_betaflight() => Self::AntiGravity,
            43u32 if fw.is_betaflight() => Self::DynLpf,
            44u32 if fw.is_betaflight() => Self::RxSpektrumSpi,
            45u32 if fw.is_betaflight() => Self::DshotRpmTelemetry,
            46u32 if fw.is_betaflight() => Self::RpmFilter,
            47u32 if fw.is_betaflight() => Self::DMin,
            48u32 if fw.is_betaflight() => Self::AcCorrection,
            49u32 if fw.is_betaflight() => Self::AcError,
            50u32 if fw.is_betaflight() => Self::DualGyroScaled,
            51u32 if fw.is_betaflight() => Self::DshotRpmErrors,
            52u32 if fw.is_betaflight() => Self::CrsfLinkStatisticsUplink,
            53u32 if fw.is_betaflight() => Self::CrsfLinkStatisticsPwr,
            54u32 if fw.is_betaflight() => Self::CrsfLinkStatisticsDown,
            55u32 if fw.is_betaflight() => Self::Baro,
            56u32 if fw.is_betaflight() => Self::GpsRescueThrottlePid,
            57u32 if fw.is_betaflight() => Self::DynIdle,
            58u32 if fw.is_betaflight() => Self::FeedforwardLimit,
            59u32 if fw.is_betaflight() => Self::Feedforward,
            60u32 if fw.is_betaflight() => Self::BlackboxOutput,
            61u32 if fw.is_betaflight() => Self::GyroSample,
            62u32 if fw.is_betaflight() => Self::RxTiming,
            63u32 if fw.is_betaflight() => Self::DLpf,
            64u32 if fw.is_betaflight() => Self::VtxTramp,
            65u32 if fw.is_betaflight() => Self::Ghst,
            66u32 if fw.is_betaflight() => Self::GhstMsp,
            67u32 if fw.is_betaflight() => Self::SchedulerDeterminism,
            68u32 if fw.is_betaflight() => Self::TimingAccuracy,
            69u32 if fw.is_betaflight() => Self::RxExpresslrsSpi,
            70u32 if fw.is_betaflight() => Self::RxExpresslrsPhaselock,
            71u32 if fw.is_betaflight() => Self::RxStateTime,
            72u32 if fw.is_betaflight() => Self::GpsRescueVelocity,
            73u32 if fw.is_betaflight() => Self::GpsRescueHeading,
            74u32 if fw.is_betaflight() => Self::GpsRescueTracking,
            75u32 if fw.is_betaflight() => Self::Attitude,
            76u32 if fw.is_betaflight() => Self::VtxMsp,
            _ => {
